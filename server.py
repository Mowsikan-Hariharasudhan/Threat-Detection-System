...
def handle_threat(threat):
    ...
    - Saves to DB (or memory fallback)
    - Emits socket event
    - Sends email notification
    """
    print(f"Handling threat: {threat['scenario']['type']} (Score: {threat['risk_score']})")
        ...
        with app.app_context():
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden;">
                ...
                <div style="background-color: #f8fafc; padding: 16px; text-align: center; color: #94a3b8; font-size: 12px;">
                    Generated by CyberGuard AI • {datetime.now().year}
                </div>
            </div>
            """
            
            mail.send(msg)
            print(f"📧 Threat notification sent to {recipient}")
